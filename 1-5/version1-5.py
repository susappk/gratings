...
def fuc(aa,mode):
    ...
    lambda0=1
    d=aa*lambda0
    theta=np.arcsin(lambda0/d/2)
    #################### #periodic structure
    h=np.array([0.1*lambda0])
    epsilon=np.array([(1.45+7.1j)**2,1.0**2])
    ...
    #theta orders
    thetaR=np.arcsin(alpha/k[1])/np.pi*180
    ...
    HT=np.dot(T22,np.exp(-1j*beta[p+1,N]*yp[0])*deltan0)
    T12=(T[:(2*N+1),(2*N+1):])
    HR=np.dot(T12,T22)
    HR=np.exp(-1j*beta[p+1,:]*yp[0])*np.dot(HR,np.exp(-1j*beta[p+1,N]*yp[0])*deltan0)
    if mode == 'tm':
...
kk=50
yitatm=np.arange((kk),dtype=complex)
yitate=np.arange((kk),dtype=complex)
ratio=np.linspace(0.55,45,kk)
for i in np.arange(kk):
    yitatm[i]=fuc(ratio[i],'tm')
    yitate[i]=fuc(ratio[i],'te')
plt.figure(figsize=(12,12))
plt.plot(ratio,np.real(yitatm),label="$TM$",color="b")
plt.plot(ratio,np.real(yitate),label="$TE$",color="g")
plt.xlabel("d/lambda0")
plt.ylabel("yita")
plt.title("te&tm -1 littrow diffraction efficiencies")
plt.legend()
plt.show()
